...
def mouse_fusao_Eth(delta_x_mi,delta_y_mi,x_ta,y_ta,theta_ta,theta_mi,r_mi):
...
#          " dx_mc: ","{:10.4f}".format(a[2][0])," dy_mc: ","{:10.4f}".format(a[3][0]))
    ...
    for i in np.arange(0,nm*2,2):
        ...
        if (i == 0):
            ...
            Eij_m = np.concatenate((Eij_m,temp),0)
    ...
    if Eth > 0:
        ...
        ri = ri.reshape(ri.size,1)
    
        ri = ri[Eij_m < Eth]
        
        ###########################################################################
        
        B = np.zeros((ri.size*2,3))
        ...
        for i in ri:
            ...
            if (j == 0):
                ...
                B = np.concatenate((B,temp),0)
        
        b = np.zeros((ri.size*2,1))
        ...
        [x1,x2,x3,x4] = np.linalg.lstsq(B,b)
        #u = np.linalg.pinv(A)@a
        #u = np.linalg.lstsq(A,a)
