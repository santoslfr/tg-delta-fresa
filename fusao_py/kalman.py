...
def kalman(delta_xr,delta_yr,delta_thetar,x_ta,y_ta,theta_ta,xtc,ytc,thetac,P_ta):
    ...
    Rt = np.diag(np.diag(wt@(wt.transpose())))
    
    # Qt = np.diag(np.array([ sigma_t_wx^2 , sigma_t_wy^2 , sigma_t_wz^2]))
    
    Qt = np.diag(np.diag(vt@(vt.transpose())))
...
#    theta_t = theta_ta + delta_thetar
    ...
    Ft = np.array([[1 , 0 , (delta_xr*np.cos(theta_ta) - delta_yr*np.sin(theta_ta))/theta_ta],
                   [0 , 1 , (delta_xr*np.sin(theta_ta) + delta_yr*np.cos(theta_ta))/theta_ta],
                   [0 , 0 ,  1 + delta_thetar/theta_ta                                           ]])
    ...
    Kt = (P_pre@(Ht.transpose()))@np.linalg.inv((Ht@P_pre@(Ht.transpose()) + Rt))
    ...
    #print(" xm_p: ",x_pre," ym_p: ",y_pre," thetam_p : ",theta_pre*((360)/(2*np.pi)))
